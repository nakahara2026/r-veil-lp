...
新しいセッションで作業を始める前に、このファイルと`design.md`を必ず読むこと。
...
| ファイル | 役割 |
|---|---|
| `design.md` | **最重要**。ブランドの背景・コンセプト、デザイントークン、全セクションのワイヤーフレーム、コンポーネント一覧、アセット状況、次セッションTODOをまとめた設計書。実装前に必ず目を通す。作業のたびに更新すること（後述） |
| `top.png`（プロジェクトルート） | デザインカンプの実物（2114×29505pxのフルページスクリーンショット）。**gitignore対象、リポジトリには含まれない**のでローカルにのみ存在。Pythonの`PIL`でクロップ＋グリッド重ねをして実測するのが基本ワークフロー（design.md参照）。新規セクション実装・レイアウトの疑義がある場合は必ずこの画像を実測してから着手する（目測で実装すると手戻りが発生しやすい） |
...
`design.md`のセクション10「次セッションTODO」に、直近の未完了タスクが優先度順でまとめてある。作業開始時はまずそこを確認する。
...
- **フォルダ構成**：
  ...
  src/
    ...
    components/
      ...
      Placeholder.astro       … 未支給画像用の仮表示ボックス
  public/
...
1. `design.md`を通読する（特にセクション2「ブランド背景・コンセプト」とセクション10「次セッションTODO」）。
...
6. 作業が一区切りついたら、`design.md`を実装内容に合わせて更新する（実装とドキュメントを常に同期させる運用）。
