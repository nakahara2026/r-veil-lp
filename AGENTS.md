...
新しいセッションで作業を始める前に、このファイル・`design.md`・`top.md`（トップページ作業時）を必ず読むこと。
...
- **現状のフェーズ**：デザインカンプ（`top.png`）をベースにしたトップページの実装がほぼ完了。下層ページはまだ未着手。
- **設計書の構成**：`design.md`（全ページ共通ルール）と`top.md`（トップページ固有の構成・文言）の2ファイルに分割済み。**新規に下層ページを作る際は、`design.md`のルールを踏まえたうえで、そのページ専用の`○○.md`（例：`product.md`）を`top.md`と同じ構成で新規作成すること**。
...
| `top.png`（プロジェクトルート） | デザインカンプの実物（フルページスクリーンショット）。**gitignore対象、リポジトリには含まれない**のでローカルにのみ存在。Pythonの`PIL`でクロップ＋グリッド重ねをして実測するのが基本ワークフロー（design.md 6.2参照）。新規セクション実装・レイアウトの疑義がある場合は必ずこの画像を実測してから着手する（目測で実装すると手戻りが発生しやすい） |
...
`top.md`のセクション7「次セッションTODO」に、直近の未完了タスクが優先度順でまとめてある。トップページの作業を始める際はまずそこを確認する。
...
- **フォルダ構成**：
  ```
  src/
    pages/index.astro       … トップページ本体（全セクションのHTML+CSS+JS、単一ファイルに集約）
  ...
  public/
    images/                 … 全画像アセット（コート/バッグ商品写真、背景写真、アイコン、ロゴ等）
...
1. `design.md`を通読する（特にセクション2「ブランド背景・コンセプト」）。トップページを触るなら`top.md`も通読する（特にセクション7「次セッションTODO」）。
...
6. 作業が一区切りついたら、変更内容に応じて`design.md`（全ページ共通ルールの変更）・`top.md`（トップページ固有の変更）を実装内容に合わせて更新する（実装とドキュメントを常に同期させる運用）。下層ページを新規作成した場合は、そのページ専用の`○○.md`も同様に作成・更新する。
